...
\begin{frame}{Nematic Helmholtz-Korteweg equation\footnotemark}
  ...
  \end{itemize}
  \footnotetext{\tiny P.E. Farrell, U. Zerbinati, \emph{Time-harmonic waves in Korteweg and nematic-Korteweg fluids.} arXiv, 2024.}
\end{frame}
...
\begin{frame}{Continuous Analysis: compactness}
  \begin{minipage}{0.83\textwidth}
    ...
    \begin{align*}
      \Vert K u \Vert_{H^2(\Omega)} &= \sup_{v \in X \setminus \{ 0 \}} \frac{\vert (Ku,v)_{H^2(\Omega)} \vert}{\Vert v \Vert_{H^2(\Omega)}} \\
...
\begin{frame}{Gaussian pulse}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.1]
      ...
      \node<2-> at (-1,0) {\includegraphics[width=0.23\textwidth,keepaspectratio]{figures/GPIM-X.png}};
      \node<2-> at (2,0) {\includegraphics[width=0.23\textwidth,keepaspectratio]{figures/GPIM-XY.png}};
      \node<2-> at (5,0) {\includegraphics[width=0.23\textwidth,keepaspectratio]{figures/GPIM-Y.png}};
      \node at (-4,-1.75) {\footnotesize $\beta = 0$};
      \node<2-> at (-1,-1.75) {\footnotesize $\beta = 5\cdot 10^{-3}$};
      \node<2-> at (2,-1.75) {\footnotesize $\beta = 5\cdot 10^{-3}$};
      \node<2-> at (5,-1.75) {\footnotesize $\beta = 5\cdot 10^{-3}$};
...
\begin{frame}{Mullen-L\"uthi-Stephen experiment\footnotemark}
  \footnotetext{\tiny M.E. Mullen, B. L\"uthi, M.J. Stephen, \emph{Sound velocity in a nematic liquid crystal}. Physics review letters, 1972.}
\end{frame}
